...
export default function SizeComparisonPage() {
	...
	const handleRenderFonts = useCallback(() => {
		if (font1Family && font1Url) {
			loadFont1(font1Family, font1Url)
				.then(() =>
					setActiveFont1({ family: font1Family, isItalic: isItalic1 })
				)
				.catch(() => {});
		}
		if (font2Family && font2Url) {
			loadFont2(font2Family, font2Url)
				.then(() =>
					setActiveFont2({ family: font2Family, isItalic: isItalic2 })
				)
				.catch(() => {});
		}
	}, [
	...
	useEffect(() => {
		...
		if (params.has("font1Url")) {
			...
			setSampleText1(
				decodeURIComponent(params.get("sampleText1") || "My Fox Jumps")
			);
			...
			setSampleText2(
				decodeURIComponent(params.get("sampleText2") || "Wily Quiz Glib")
			);
	...
	const handleItalic1Toggle = (e) => {
		...
		if (activeFont1)
			setActiveFont1((prev) => ({ ...prev, isItalic: isChecked }));
	};
	...
	const handleItalic2Toggle = (e) => {
		...
		if (activeFont2)
			setActiveFont2((prev) => ({ ...prev, isItalic: isChecked }));
	};
	...
	const handleShare = () => {
		...
		}/size-comparison?${params.toString()}`;
		...
		<>
			<div className="bg-white p-6 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-b pb-6 border-gray-200 dark:border-gray-700">
					<fieldset className="border rounded-lg p-4 border-gray-300 dark:border-gray-600">
						<legend className="text-lg font-semibold text-gray-800 px-2 dark:text-gray-100">
							Font 1
						</legend>
						<div className="grid grid-cols-1 gap-4">
							...
							<div>
								...
								<input
									type="text"
									value={font1Family}
									onChange={(e) => setFont1Family(e.target.value)}
							...
							<div>
								...
								<input
									type="text"
									value={sampleText1}
									onChange={(e) => setSampleText1(e.target.value)}
									className="w-full p-3 border border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100"
								/>
							</div>
						...
						<div className="grid grid-cols-1 gap-4">
							...
							<div>
								...
								<input
									type="text"
									value={font2Family}
									onChange={(e) => setFont2Family(e.target.value)}
									className="w-full p-3 border border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100"
								/>
							</div>
							<div>
								...
								<input
									type="text"
									value={sampleText2}
									onChange={(e) => setSampleText2(e.target.value)}
									className="w-full p-3 border border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100"
								/>
							</div>
							<div className="flex items-center">
				...
				<fieldset className="mt-4 border-b pb-4 border-gray-200 dark:border-gray-700">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
						...
						<div className="flex items-center">
							<input
								id="show-center-line-comparison"
								type="checkbox"
							...
							/>
							<label
								htmlFor="show-center-line-comparison"
								className="ml-2 block text-sm text-gray-900 dark:text-gray-300"
							>
								Show Center Line
							</label>
				...
				<div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-2 items-center">
					<button
						onClick={handleRenderFonts}
						disabled={isLoading1 || isLoading2}
						className="w-full sm:col-span-2 bg-blue-600 text-white px-6 py-3 rounded-md font-semibold hover:bg-blue-700 disabled:bg-gray-400"
					>
						{isLoading1 || isLoading2 ? "Loading Fonts..." : "Render Fonts"}
					</button>
					<button
						onClick={setSampleFonts}
						className="w-full bg-gray-300 text-gray-700 px-6 py-3 rounded-md font-semibold hover:bg-gray-400 dark:bg-gray-600 dark:text-gray-300 dark:hover:bg-gray-700"
					>
						Try Samples
					</button>
					<button
						onClick={handleShare}
						disabled={!activeFont1 && !activeFont2}
						className="w-full bg-green-500 text-white px-6 py-3 rounded-md font-semibold hover:bg-green-600 disabled:bg-gray-400"
					>
						Share
					</button>
					{copySuccess && (
