...
export default function AlignmentCheckPage() {
  ...
  useEffect(() => {
    ...
    if (params.has('fontUrl') && params.has('fontFamily')) {
      ...
      setSampleText(decodeURIComponent(params.get('sampleText') || 'Typo WAG jpqgy'));
  ...
  const setSample = () => {
    setFontUrl('https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,500;1,700;1,900&display=swap');
    ...
    <>
      <div className="bg-white p-6 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700">
        <fieldset className="border-b pb-4 border-gray-200 dark:border-gray-700">
            <legend className="text-lg font-semibold text-gray-800 mb-2 dark:text-gray-100">Font Source</legend>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">Font URL</label>
                <input type="text" value={fontUrl} onChange={(e) => setFontUrl(e.target.value)} className="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">Font Family Name</label>
                <input type="text" value={fontFamily} onChange={(e) => setFontFamily(e.target.value)} className="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" />
              </div>
            </div>
          </fieldset>

          <fieldset className="mt-4 border-b pb-4 border-gray-200 dark:border-gray-700">
            <legend className="text-lg font-semibold text-gray-800 mb-2 dark:text-gray-100">Display Options</legend>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-300">Sample Text</label>
                  <input type="text" value={sampleText} onChange={(e) => setSampleText(e.target.value)} className="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" />
              </div>
              <div>
              ...
              <div>
                ...
                <div className="inline-flex rounded-md shadow-sm"><button type="button" onClick={() => setDisplayMode('comparison')} className={`px-4 py-2 text-sm font-medium transition-colors rounded-l-lg border border-gray-300 ${displayMode === 'comparison' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600'}`}>Comparison</button><button type="button" onClick={() => setDisplayMode('normal')} className={`px-4 py-2 text-sm font-medium transition-colors border-t border-b border-gray-300 ${displayMode === 'normal' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600'}`}>Normal</button><button type="button" onClick={() => setDisplayMode('italic')} className={`px-4 py-2 text-sm font-medium transition-colors rounded-r-md border border-gray-300 ${displayMode === 'italic' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600'}`}>Italic</button></div>
              </div>
              <div>
              ...
              <div className="flex items-center"><input id="show-background" type="checkbox" checked={showBackground} onChange={(e) => setShowBackground(e.target.checked)} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 dark:border-gray-600" /><label htmlFor="show-background" className="ml-2 block text-sm text-gray-900 dark:text-gray-300">Show Background</label></div>
            </div>
          </fieldset>
