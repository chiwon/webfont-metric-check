export default function FontDisplay({ fontFamily, sampleText, fontSize, displayMode, lineHeight, overflowHidden, showCenterLine, showBackground }) {
  ...
  const renderText = (style, isOverlay) => {
        ...
        {weights.map(weight => (
          <div key={weight}>
            ...
            <div className="relative border border-gray-300 bg-white mt-1 dark:bg-black dark:border-gray-700" style={{ fontFamily: `'${fontFamily}', sans-serif`, fontWeight: weight, lineHeight: lineHeight, fontSize: `${fontSize}px` }}>
              <span className="invisible whitespace-nowrap" style={{ fontStyle: 'normal' }} className={showBackground ? 'bg-red-400/20' : ''}>{sampleText}</span>
              {(displayMode === 'normal' || displayMode === 'comparison') && renderText('normal', false)}
              {(displayMode === 'italic' || displayMode === 'comparison') && renderText('italic', displayMode === 'comparison')}
